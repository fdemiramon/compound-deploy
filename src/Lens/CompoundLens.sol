...
contract CompoundLens {
    ...
    function cTokenMetadata(CToken cToken) public returns (CTokenMetadata memory) {
        ...
        (bool borrowCapSuccess, bytes memory borrowCapReturnData) =
            address(comptroller).call(abi.encodePacked(comptroller.borrowCaps.selector, abi.encode(address(cToken))));
        if (borrowCapSuccess) {
    ...
    function getCompBalanceMetadataExt(Comp comp, ComptrollerLensInterface comptroller, address account)
        external
    ...
    function getCompVotes(Comp comp, address account, uint32[] calldata blockNumbers)
        external
        view
