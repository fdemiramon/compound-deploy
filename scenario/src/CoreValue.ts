...
import {
  comptrollerFetchers,
  getComptrollerValue
} from "./Value/ComptrollerValue";
import {
  comptrollerImplFetchers,
  getComptrollerImplValue
} from "./Value/ComptrollerImplValue";
import {
  getUnitrollerValue,
  unitrollerFetchers
...
import {
  ...
  getCTokenDelegateValue
} from "./Value/CTokenDelegateValue";
...
import {
  ...
  interestRateModelFetchers
} from "./Value/InterestRateModelValue";
import {
  getPriceOracleValue,
  priceOracleFetchers
} from "./Value/PriceOracleValue";
import {
  getPriceOracleProxyValue,
  priceOracleProxyFetchers
} from "./Value/PriceOracleProxyValue";
import {
  getAnchoredViewValue,
  anchoredViewFetchers
} from "./Value/AnchoredViewValue";
import {
  getTimelockValue,
  timelockFetchers,
  getTimelockAddress
} from "./Value/TimelockValue";
import {
  getMaximillionValue,
  maximillionFetchers
} from "./Value/MaximillionValue";
import { getCompValue, compFetchers } from "./Value/CompValue";
import { getGovernorValue, governorFetchers } from "./Value/GovernorValue";
import {
  getGovernorBravoValue,
  governorBravoFetchers
} from "./Value/GovernorBravoValue";
...
export async function mapValue<T>(
  ...
  if (!(val instanceof type)) {
    throw new Error(
      `Expected "${
        type.name
      }" from event "${event.toString()}", was: "${val.toString()}"`
    );
  }
...
export async function getAddressV(
  world: World,
  event: Event
): Promise<AddressV> {
...
export async function getExpNumberV(
  world: World,
  event: Event
): Promise<NumberV> {
  let res = await mapValue<NumberV>(
    world,
    event,
    strToNumberV,
    getCoreValue,
...
export async function getPercentV(
  world: World,
  event: Event
): Promise<NumberV> {
  ...
  await Promise.all(
    mustArray(event).map(async e => {
      if (Array.isArray(e) && e.length === 2 && typeof e[0] === "string") {
        ...
        throw new Error(
          `Expected all string pairs for MapV from ${event.toString()}, got: ${e.toString()}`
        );
      }
    })
  );
...
const fetchers = [
  ...
  new Fetcher<{ amt: EventV }, NumberV>(
    ...
    [new Arg("amt", getEventV)],
    async (world, { amt }) =>
      new NumberV(0).sub(await getNumberV(world, amt.val))
  ),
  ...
  new Fetcher<{ amt: StringV }, PreciseV>(
    ...
    async (world, { amt }) =>
      new PreciseV(toEncodableNum(amt.val), getSigFigs(amt.val))
  ),
    ...
    async (world, { addr, slot, start, valType }) => {
      ...
      const reverse = s =>
        s
          ...
          .reverse()
          .join("");
      const storage = await world.web3.eth.getStorageAt(
        addr.val,
        slot.toNumber()
      );
      ...
      switch (valType.val) {
        ...
        case "address":
          return new AddressV(
            "0x" + padLeft(reverse(stored.slice(startVal, startVal + 40)), 40)
          );
        case "number":
        ...
        case "marketStruct":
          ...
          let collateralFactorKey =
            ...
            toBN(newKey)
              .add(toBN(1))
              .toString(16);
          let collateralFactorStr = await world.web3.eth.getStorageAt(
            addr.val,
            collateralFactorKey
          );
          ...
          let userInMarket = await world.web3.eth.getStorageAt(
            addr.val,
            newKeyTwo
          );
          ...
          let isCompKey =
            ...
            toBN(newKey)
              .add(toBN(3))
              .toString(16);
          let isCompStr = await world.web3.eth.getStorageAt(
            addr.val,
            isCompKey
          );
  ...
  >(
    ...
    * "StorageAtMapping addr:<Address> slot:<Number>, key:<address>, valType:<VToCastTo>" - Returns bytes at storage slot
    ...
    async (world, { addr, slot, key, valType }) => {
      ...
      switch (valType.val) {
        case "list(address)":
          let p = new Array(toDecimal(val))
            .fill(undefined)
            .map(async (_v, index) => {
              let newKeySha = sha3(newKey);
              let itemKey = toBN(newKeySha).add(toBN(index));
              let address = await world.web3.eth.getStorageAt(
                addr.val,
                padLeft(toHex(itemKey), 40)
              );
  ...
  new Fetcher<{}, NumberV>(
    ...
    async (world, {}) => {
      ...
      if (!invokation) {
        throw new Error(
          `Expected last invokation for "lastBlock" but none found.`
        );
      }

      if (!invokation.receipt) {
        throw new Error(
          `Expected last invokation to have receipt for "lastBlock" but none found.`
        );
      }
  ...
  new Fetcher<{}, NumberV>(
    ...
    async (world, {}) => {
      ...
      if (!invokation) {
        throw new Error(
          `Expected last invokation for "lastGas" but none found.`
        );
      }

      if (!invokation.receipt) {
        throw new Error(
          `Expected last invokation to have receipt for "lastGas" but none found.`
        );
      }
  ...
  new Fetcher<{}, NumberV>(
    ...
    [],
    async (world, {}) => {
      const { result: blockNumber }: any = await sendRPC(
        world,
        "eth_blockNumber",
        []
      );
      const { result: block }: any = await sendRPC(
        world,
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    ...
    async (world, { given, expected }) =>
      new BoolV(expected.compareTo(world, given))
  ),
  new Fetcher<
    {
      argTypes: StringV[];
