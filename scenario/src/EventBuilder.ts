...
import { AddressV, ArrayV, EventV, NumberV, StringV, Value } from "./Value";
import {
  getAddressV,
  getArrayV,
  getEventV,
...
function buildArg(
  ...
  if (!getter) {
    throw new Error(
      `Unknown ABI Input Type: ${input.type} of \`${name}\` in ${contractName}`
    );
  }
...
function getContractObjectFn(contractName, implicit) {
  if (implicit) {
    ...
    return async function getContractObject(
      world: World,
      event: Event
    ): Promise<Contract> {
...
export function buildContractEvent<T extends Contract>(
  contractName: string,
  implicit
) {
  return async world => {
    ...
    async function build<T extends Contract>(
      ...
      const fetchers = constructors.map((abi: any) => {
        let nameArg = implicit
          ...
          : [
              new Arg("name", getStringV, {
                default: new StringV(contractName)
              })
            ];
        ...
        let args = abi.inputs.map(input =>
          buildArg(contractName, input.name, input)
        );
        return new Fetcher<object, ContractData<T>>(
          `
            #### ${contractName}

            * "${contractName} ${nameArgDesc}${inputNames.join(
            " "
          )} - Build ${contractName}
      ...
      let data = await getFetcherValue<any, ContractData<T>>(
        `Deploy${contractName}`,
        fetchers,
        world,
        params
      ...
      const index =
        contractName == data.name ? [contractName] : [contractName, data.name];
    ...
    async function deploy<T extends Contract>(
      ...
      let { world: nextWorld, contract, data } = await build<T>(
        world,
        from,
        params
      );
      ...
      let abiCommands = abis
        ...
        .map((abi: any) => {
          ...
          let args = [
            new Arg(
              "contract",
              getContractObjectFn(contractName, implicit),
              implicit ? { implicit: true } : {}
            )
          ].concat(
            abi.inputs.map(input => buildArg(contractName, abi.name, input))
          );
          ...
          return new Command<object>(
            ...
            * "${eventName} ${inputNames.join(" ")}" - Executes \`${
              abi.name
            }\` function
          `,
            eventName,
            args,
            (world, from, inputs) =>
              buildOutput(world, from, abi.name, inputs, abi.outputs[0]),
...
export async function buildContractFetcher<T extends Contract>(
  world: World,
  contractName: string,
  implicit: boolean
) {
  ...
  function fetchers() {
    async function buildOutput(
      ...
      if (!builder) {
        throw new Error(
          `Unknown ABI Output Type: ${output.type} of \`${fn}\` in ${contractName}`
        );
      }
    ...
    return abis
      ...
      .map((abi: any) => {
        ...
        return new Fetcher<object, Value>(
          ...
          * "${eventName} ${inputNames.join(" ")}" - Returns the result of \`${
            abi.name
          }\` function
        `,
          eventName,
          args,
          (world, inputs) =>
            buildOutput(world, abi.name, inputs, abi.outputs[0]),
          { namePos: implicit ? 0 : 1 }
        );
      });
  }
