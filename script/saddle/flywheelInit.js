...
let writeFixture = async data => {
  await writeFile(
    `./script/saddle/fixture/${network}_borrowers.json`,
    JSON.stringify(data)
  );
};
...
let filterInitialized = async borrowersByCToken => {
  ...
  let batchSize = 75;
  console.log(
    `Calling compBorrowerIndex for borrowers in batches of ${batchSize}...\n`
  );
  for (let cTokenAddr of Object.keys(borrowersByCToken)) {
    ...
    if (Number(supplySpeed) != 0 || Number(borrowSpeed) != 0) {
      for (let borrowerChunk of getChunks(
        borrowersByCToken[cTokenAddr],
        batchSize
      )) {
        try {
          let indices = await Promise.all(
            borrowerChunk.map(async borrower => {
              return await call(Comptroller, "compBorrowerIndex", [
                cTokenAddr,
                borrower
              ]);
            })
          );
          let uninitialized = borrowerChunk.filter(
            (borrower, i) => Number(indices[i]) == 0
          );
          res[cTokenAddr] = res[cTokenAddr]
            ? res[cTokenAddr].concat(uninitialized)
            : uninitialized;
        } catch (e) {
