module.exports = {
  ...
  extra_build_files: [
    process.env["EXTRA_BUILD_FILES"] || "remote/*.json",
    "tests/LegacyBuilds/*.json"
  ], // Additional build files to deep merge
  // coverage_dir: "coverage",                              // Directory to place coverage files
  // coverage_ignore: [],                                   // List of files to ignore for coverage
  contracts:
    process.env["SADDLE_CONTRACTS"] ||
    "{contracts,contracts/**,tests/Contracts}/*.sol",
  ...
  networks: {
    // Define configuration for each network
    dev: {
      providers: [{ http: "http://127.0.0.1:8545" }],
      web3: {
        // Web3 options for immediate confirmation in development mode
        gas: [{ env: "GAS" }, { default: "6700000" }],
        gas_price: [{ env: "GAS_PRICE" }, { default: "12000000000" }],
        options: {
      ...
      accounts: [
        // How to load default account for transactions
    ...
    test: {
      ...
      web3: {
        gas: [{ env: "GAS" }, { default: "20000000" }],
        gas_price: [{ env: "GAS_PRICE" }, { default: "12000000002" }],
        options: {
          transactionConfirmationBlocks: 1,
          transactionBlockTimeout: 5
        }
      },
      accounts: [{ env: "ACCOUNT" }, { unlocked: 0 }]
    },
    goerli: {
      providers: [
        { env: "PROVIDER" },
    ...
    sepolia: {
      ...
      web3: {
        gas: [{ env: "GAS" }, { default: "6700000" }],
        gas_price: [{ env: "GAS_PRICE" }, { default: "246000000000" }],
        options: {
          transactionConfirmationBlocks: 1,
          transactionBlockTimeout: 5
        }
      ...
      accounts: [
        { env: "ACCOUNT" },
        { file: "~/.ethereum/sepolia" } // Load from given file with contents as the private key (e.g. 0x...)
      ]
    },
    rinkeby: {
      providers: [
    ...
    mainnet: {
      ...
      web3: {
        gas: [{ env: "GAS" }, { default: "6000000" }],
        gas_price: [{ env: "GAS_PRICE" }, { default: "45000000000" }],
        options: {
          transactionConfirmationBlocks: 1,
          transactionBlockTimeout: 5
        }
  ...
  read_network_file: network => {
    ...
    return util
      .promisify(fs.readFile)(networkFile)
      .then(json => {
